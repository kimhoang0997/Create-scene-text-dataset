...
def preprocess():
    ...
    parser.add_argument('-o', '--output', metavar="output-path", default="output", help='folder that stores output files [default=output]')
    ...
    parser.add_argument('-n', '--numberwords', type=int, default=50000, help='number of words in trainning dataset [default=50000]')
        ...
        for word_index, word in  enumerate(word_bank):
            ...
            if augmentation:
                random_StrAug = Random_StrAug(using_aug_types = ['warp', 'geometry', 'blur', 'noise', 'camera', 'pattern', 'process', 'weather'],
                                    prob_list = [0.4, 0.2, 0.2, 0.1, 0.2, 0.1, 0.1, 0.1])
...
def main():
    ...
    word_list = []
    with open(config["dictionary"],"r", encoding="utf-8") as dict_file:
        words = dict_file.readlines()
        for word in words:
            word_list.append(word.strip())

    word_bank = choices(word_list,k= args.numberwords)
